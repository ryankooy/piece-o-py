...
def grid_and_mines(gridsize=gridsize, minerange=minerange):
...
def print_grid(show, gridsize=gridsize):
  xgrid, ygrid = gridsize
...
def clear(x, y, showgrid, grid, mines, markedmines):
  if showgrid[(x, y)] == '.':
    xychar = str(sum(1 for xx in (x-1, x, x+1)
      for yy in (y-1, y, y+1)
      if (xx, yy) in mines))
    if xychar == '0': xychar = '.'
    showgrid[(x, y)] = xychar
...
if __name__ == '__main__':
    ...
    if inp:
      if inp[0] == 'm':
        ...
        if (x, y) in mines | markedmines:
          print('\nKABLOOMIES! You hit a mine and are dieded.\n')
  ...
  print('\nYou found %i and avoided %i of the %i mines.' % (
